...
const MyProducts = () => {
  ...
  const { data: products = [], refetch } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(
        `https://phone-baba-server.vercel.app/products?email=${user.email}`
      );
  ...
  const deleteProduct = (id) => {
    fetch(`https://phone-baba-server.vercel.app/products/${id}`, {
      method: "DELETE",
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  const adsProduct = (id) => {
    console.log(id);
    fetch(`https://phone-baba-server.vercel.app/product/ads/${id}`, {
      method: "PUT",
    })
      .then((res) => res.json())
      .then((result) => {
