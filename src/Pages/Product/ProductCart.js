...
const ProductCart = ({ product, setBooking }) => {
  ...
  useEffect(() => {
    fetch(
      `https://phone-baba-server.vercel.app/users?email=${product.sellerEmail}`
    )
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleReportedItem = (product) => {
    // console.log(product);
    fetch(
      `https://phone-baba-server.vercel.app/report/product/${product._id}`,
      {
        method: "PUT",
      }
    )
  ...
  const handleWishItem = (product) => {
    ...
    fetch("https://phone-baba-server.vercel.app/wish", {
      method: "POST",
      headers: {
